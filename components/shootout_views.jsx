...
_.extend(ScoringState.prototype, {
  ...
  setDefaultPlayers: function() {
    this.activePlayerScore = 
      this.inactivePlayerScore =
      this.leadPlayerScore = 
      this.behindPlayerScore =
      this.evenPlayerScore =
      this.oddPlayerScore =
      this.playerScores[0];
  },
  ...
  gameOver: function() {
    return (this.leadPlayerScore.score > this.behindPlayerScore.maxPossibleScore) || 
      (this.gameState.question_index === this.gameState.question_count);
  },
  ...
  endgameRound: function() {
    ...
    this.gameState.questions.forEach(function(question, i) {
      ...
      if( (players[0].score === players[1].maxPossibleScore) ||
          (players[1].score === players[0].maxPossibleScore) ) {
        round.push(i + 1);
      }
...
app.Views.Container = React.createClass({
  ...
  render: function() {
    ...
    return <div className="container">
      ...
      />
      <app.Views.Players players={players} />
    </div>
  },
...
app.Views.Message = React.createClass({
  ...
  generateMessage: function() {
    ...
    if(scoringState.gameOver()) {
      if(scoringState.gameTied()) {
        ...
        return scoringState.leadPlayerScore.player.name + 
          " has won " +
          this.scoreMessage() +
          ".";
      }
    ...
    if(scoringState.winImminent() && !scoringState.lastRound()) {
      ...
      if(scoringState.aboutToLose()) {
        ...
        messages.push(
          ...
          ", if you get one more question wrong, " +
          "or if " +
          scoringState.leadPlayerScore.player.name +
          " answers one more question correctly, " +
          scoringState.leadPlayerScore.player.name +
          " will win the game."
        );
        ...
        messages.push(
          scoringState.leadPlayerScore.player.name +
          ", if you get this question right, you win."
        );
      }
    }
    ...
    if(scoringState.lastRound()) {
      if(scoringState.evenPlayerActive()) {
        ...
        if(scoringState.gameTied()) {
          ...
          messages.push(
            scoringState.behindPlayerScore.player.name +
            ", you must answer this question correctly to stay in the game and force a tiebreaker."
          );
        }
  ...
  scoreMessage: function() {
    return this.props.scoring_state.leadPlayerScore.score + 
      " to " +
      this.props.scoring_state.behindPlayerScore.score;
  },
